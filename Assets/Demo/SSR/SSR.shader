Shader "ImageEffect/SSR"
{
		...
		SubShader
	{
		...
		float4 reflection(v2f i) : SV_Target
		{
			...
			float a = pow(min(1.0, 100000.0 / length(calcTimes)), 2.0);
		...
		float4 xblur(v2f i) : SV_Target
		{
			...
			float4 col = tex2D(_ReflectionTexture, uv) * weight[0];
			for (int j = 1; j < 5; ++j)
			{
				col += tex2D(_ReflectionTexture, uv + float2(offset[j], 0) * size) * weight[j];
				col += tex2D(_ReflectionTexture, uv - float2(offset[j], 0) * size) * weight[j];
			}

			float depth = SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, uv);
			float depthRight = SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, uv + float2(1, 0) * size);
			float depthLeft = SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, uv - float2(1, 0) * size);

			if (depth <= 0) return tex2D(_ReflectionTexture, uv);

			float4 outColor = (abs(depthLeft - depthRight) > _BlurThreshold) ? tex2D(_ReflectionTexture, uv) : col * (1 - smooth) + tex2D(_ReflectionTexture, uv) * smooth;
		...
		float4 yblur(v2f i) : SV_Target
		{
			...
			for (int j = 1; j < 5; ++j)
			{
				col += tex2D(_ReflectionTexture, uv + float2(0, offset[j]) * size) * weight[j];
				col += tex2D(_ReflectionTexture, uv - float2(0, offset[j]) * size) * weight[j];
			}

			float depth = SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, uv);
			float depthTop = SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, uv + float2(0, 1) * size);
			float depthBtm = SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, uv - float2(0, 1) * size);

			if (depth <= 0) return tex2D(_ReflectionTexture, uv);

			float4 outColor = (abs(depthTop - depthBtm) > _BlurThreshold) ? tex2D(_ReflectionTexture, uv) : col * (1 - smooth) + tex2D(_ReflectionTexture, uv) * smooth;
